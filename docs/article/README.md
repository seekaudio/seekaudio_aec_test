...
AI 对话玩具、智能音箱、可视门铃、楼宇对讲、会议终端——只要设备需要"边放音、边收音"的全双工语音交互，扬声器信号就会经过空气路径回灌进麦克风，形成回声。回声消除（AEC）因此成为端侧语音链路中计算量最重、也最影响体验的一环。而在 ESP32-S3 这类端侧 MCU 上，工程师面对的是算力、内存、功耗的三重约束：AEC 占掉的每一个百分点 CPU，都是从唤醒词识别、网络协议栈和电池续航里挤出来的。
...
- **双档降噪可选**：创建时一个参数选择后级降噪引擎——标准 NS（基于 WebRTC NS，内存占用低）或 AI 降噪（轻量级神经网络模型，对非平稳噪声抑制更强）；
...
SeekAudio AEC 采用"传统信号处理 + 轻量级 AI 神经网络"的混合架构（见框图，绿色块为传统算法，紫色块为 AI 训练模型）：

- **自适应滤波器（NLMS/RLS）** 承担线性回声估计——这是数学上最高效的部分，交给传统算法；
- **双讲检测（DTD）** 由轻量级 AI 神经网络模型实现，实时判别近端说话状态并控制滤波器步长，避免双讲期间滤波器发散、近端语音被误消；
- **残余回声抑制（NLP）** 同样由轻量级 AI 模型承担，清除线性滤波无法覆盖的非线性残余（扬声器失真、机壳振动等）；
- **末级降噪**在 WebRTC NS 与 AI 降噪之间由用户配置二选一。

低算力的关键在于分工，而非缩水：线性回声估计这类数学上最高效的环节交给传统算法，AI 只以轻量级专用小模型的形态出现在判决（DTD）与非线性残余（NLP）这些传统方法难以胜任的环节——这正是三个 AI 模型加起来仍只占约三成单核算力的原因。
...
评测框架遵循**微软 AEC Challenge**（ICASSP 国际声学回声消除挑战赛）：测试素材取自其公开数据集，场景划分（远端单讲 / 近端单讲 / 双讲）遵循其评测方法。在该框架下采用两套互补的指标体系：其一是 **AECMOS 感知质量模型**——基于 ITU-T P.831 / P.808 主观评测体系的人工打分数据训练，输出 1–5 分的 MOS 感知分，是当前回声消除领域广泛使用的客观感知指标；其二是经典的 **ERLE**（回声返回损耗增益）信号指标，直接度量回声能量被压低的分贝数。感知与信号两条证据链互相印证。
...
1. **省出 32–35 个百分点的单核 CPU，换来整机稳定**。同档配置下 CPU 负载从 60.5%/73.2% 降到 28.1%/37.8%，约三分之一颗单核被释放出来。这些余量留给 Wi-Fi/蓝牙协议栈、唤醒引擎和业务逻辑，玩具在联网、对话、动作并发时不再因算力见顶而卡顿、断连或重启——卡顿、断连、重启正是语音体验崩坏最直观的形态。
2. **孩子可以随时打断玩具说话**。AI 玩具的核心交互是全双工对话：玩具播报时，孩子的插话必须被听见。双讲场景 AECMOS DT Echo 达 4.19/4.25 分——回声被压住的同时，近端童声得到保护，打断（barge-in）体验自然流畅。
3. **双讲状态输出让"打断"再进化：音量自动回退（ducking）**。库可选输出精准的四状态分类，其中"双讲"就是"孩子正在插话"的实时信号。设备拿到信号后将播放音量迅速调低：扬声器一小声，麦克风里的回声量级立刻下降，同时小信号下扬声器的非线性失真也更小——双讲段的回声消除与近端拾取质量随之跃升，云端 ASR 识别率直接受益。这个降音过程对孩子几乎无感：插话的那一刻，注意力已从"听"切换到"说"，自己的说话声天然掩蔽了播放声；配合快降慢升的平滑曲线，这是一条几乎零副作用就能显著提升对话体验的通路。这正是智能音箱行业的成熟做法（barge-in ducking），落地前提是双讲检测足够精准——而本库的 DTD 由轻量级 AI 模型驱动，且按需启用、不用零开销。
4. **送往云端的音频更干净，大模型更少"听错"**。远端单讲 ERLE 高出基线约 12 dB、残余回声低 8–12 dBFS，玩具喇叭自己的声音被更彻底地从上行音频中剔除，云端 ASR 与大模型的识别准确率直接受益。
5. **工业设计不再被声学绑架**。回声消除能力弱的方案，常要靠把喇叭和麦克风的物理距离拉远来"硬扛"回声，玩具的外形因此受限。更强的 AEC 允许喇叭与麦克风靠得更近，外形、尺寸、结构的创新空间显著放大。
6. **单麦即可，不必为弥补前端质量加第二颗麦**。esp-sr 体系下提升拾音质量的标准路径是双麦 BSS——官方基准显示全双工场景下该管线 CPU 占用高达单核 81%–93%，还要多一颗麦克风的 BOM，并满足双麦间距 4–6.5 cm、水平轴线布置的结构约束，这对小体积异形玩具几乎是外形否决项。SeekAudio AEC 单麦实测 ERLE 21.5/29.4 dB、CPU 仅 28%–38%，对以近场交互为主的 AI 玩具，一颗麦克风即可交付完整体验——BOM、CPU、结构设计三头都省。
7. **实打实的功耗优势，还留了"降频"这张牌**。按乐鑫数据手册典型电流估算，芯片数字部分平均电流可省约 13 mA（约 23%，估算依据见文末注）；更重要的是 2.6–3.6 倍的实时余量允许把主频降到 160 MHz 继续压低功耗——基线 B2（RTF 1.37×）在 160 MHz 下已无法实时运行，根本没有这个选项。

AI 玩具的退货，大多不是因为外观或价格，而是语音体验——听不清、听错、不能打断、对话卡顿。上面每一项能力都直接作用于这条体验链：更稳的整机、可随时打断且越打断越清晰的对话、更高的识别率、更自由的外形、更低的成本与功耗。语音体验立住了，退货率才能实质性地降下来。
