...
- **Two selectable noise-suppression tiers**: a single parameter at creation time selects the post NS engine — standard NS (based on WebRTC NS, low memory footprint) or AI noise reduction (a lightweight neural model, stronger on non-stationary noise);
...
SeekAudio AEC uses a hybrid "classic signal processing + lightweight AI neural network" architecture (see the diagram: green blocks are classic algorithms, purple blocks are trained AI models):
...
- **Double-talk detection (DTD)** is implemented by a lightweight AI neural model that tracks the near-end speaking state in real time and controls the filter step size, preventing filter divergence and near-end speech being cancelled during double-talk;
- **Residual echo suppression (NLP)** is likewise handled by a lightweight AI model, removing the nonlinear residual that linear filtering cannot cover (loudspeaker distortion, enclosure vibration, etc.);
- The **final noise-suppression stage** is user-configurable between WebRTC NS and AI noise reduction.

The key to the low compute is division of labor, not shrinkage: the mathematically most efficient stage - linear echo estimation - goes to classic algorithms, while AI appears only as lightweight, purpose-built small models at the stages classic methods handle poorly: decisions (DTD) and the nonlinear residual (NLP). That is why all three AI models together still take only about a third of a single core.
...
The evaluation framework follows the **Microsoft AEC Challenge** (the ICASSP acoustic echo cancellation challenge): the test material comes from its public dataset and the scene partitioning (far-end single-talk / near-end single-talk / double-talk) follows its methodology. Within that framework, two complementary metric systems are used: the first is the **AECMOS perceptual model** — trained on human ratings collected under the ITU-T P.831 / P.808 subjective-testing framework, outputting a 1–5 MOS score, the objective perceptual metric most widely used in the AEC field; the second is the classic **ERLE** (echo return loss enhancement) signal metric, which directly measures how many decibels the echo energy is pushed down. The perceptual and signal evidence chains corroborate each other.
...
1. **32–35 percentage points of a single core freed, converted into whole-device stability.** At the same NS tier, CPU load drops from 60.5%/73.2% to 28.1%/37.8% — about a third of a core released. That headroom goes to the Wi-Fi/BT stack, the wake engine and application logic, so the toy no longer stutters, drops connections or reboots when networking, talking and moving at the same time — and stutters, drops and reboots are the most visible form of a broken voice experience.
...
3. **The double-talk state output takes barge-in one step further: automatic volume ducking.** The library optionally outputs a precise four-state classification, and the "double-talk" state is a real-time signal that the child is interjecting. On that signal the device quickly turns the playback volume down: with the loudspeaker quieter, the echo level at the microphone drops immediately, and the speaker's nonlinear distortion shrinks at low drive as well - double-talk echo removal and near-end pickup quality jump, and cloud ASR accuracy benefits directly. The ducking is nearly imperceptible to the child: at the moment of interjection, attention has already switched from listening to speaking, and one's own voice naturally masks the playback; with a fast-attack / slow-release ramp this is a near-zero-side-effect path to a visibly better conversation. It is the smart-speaker industry's proven practice (barge-in ducking), and its prerequisite is a sufficiently accurate DTD - which in this library is driven by a lightweight AI model, lazily enabled, and zero-overhead when unused.
4. **Cleaner audio to the cloud; the LLM mishears less.** With ~12 dB higher FE single-talk ERLE and 8–12 dBFS lower residual echo, the toy's own loudspeaker output is removed far more thoroughly from the uplink, directly benefiting cloud ASR and LLM accuracy.
5. **Industrial design is no longer hostage to acoustics.** Solutions with weak echo cancellation often resort to physically separating the speaker and microphone to "brute-force" the echo down, constraining the toy's form. Stronger AEC lets the speaker and microphone sit close together, greatly expanding the freedom of shape, size and structure.
...
7. **A real power advantage, plus the "downclocking" card.** Estimated from typical currents in the Espressif datasheet, the chip's digital-domain average current drops by about 13 mA (~23%; see the note at the end). More importantly, the 2.6–3.6× real-time margin allows dropping the clock to 160 MHz for further savings — baseline B2 (RTF 1.37×) cannot run in real time at 160 MHz at all.

AI toys are mostly returned not over looks or price, but over the voice experience - not hearing clearly, mishearing, not being able to interrupt, stuttering dialogue. Every capability above acts directly on that experience chain: a more stable device, conversations that can be interrupted at any time and get clearer when interrupted, higher recognition accuracy, freer industrial design, lower cost and power. When the voice experience holds up, the return rate comes down for real.
